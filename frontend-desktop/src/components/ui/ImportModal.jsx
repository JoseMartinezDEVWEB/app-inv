...
import { parseExcel, parsePDF, processWithAI } from '../../services/importService';
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            if (extension === 'xlsx' || extension === 'xls') {
                ...
                addLog("Leyendo archivo PDF...");
                rawData = await parsePDF(file);
            } else {
                throw new Error("Formato no soportado");
            }

            addLog("Analizando datos con IA (esto puede tardar unos segundos)...");

            // Guardar API Key para futuros usos
            if (apiKey) localStorage.setItem('gemini_api_key', apiKey);

            const processedData = await processWithAI(rawData, apiKey);
